 ...
 */
export function convertToExcelFormat(records) {
 ...
 */
export function downloadExcelFile(records, filename = 'login_records.csv') {
    try {
 ...
 */
export function generateHTMLTable(records) {
    const excelData = convertToExcelFormat(records);
    const headers = Object.keys(excelData[0] || {});
 ...
 */
export function storeLoginRecord(record) {
    try {
 ...
 */
export function clearLoginRecords() {
    try {
 ...
 */
export function getLoginStatistics(records) {
    return {
        ...
        byProvider: records.reduce((acc, r) => {
            acc[r.authMethod] = (acc[r.authMethod] || 0) + 1;
            return acc;
        }, {}),
        byRole: records.reduce((acc, r) => {
...
export function searchRecords(records, query) {
    ...
    return records.filter((record) => record.email.toLowerCase().includes(lowercaseQuery) ||
